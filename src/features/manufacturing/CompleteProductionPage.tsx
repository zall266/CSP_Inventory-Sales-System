  ...
  return (
    <div>
      ...
      <Card className="mt-5 p-5">
        ...
        <p className="mt-1 text-sm text-slate-600">Material allocated to this production session, not current warehouse stock. Enter physical remaining — the system calculates actual used.</p>
            ...
            return (
              <div key={draft.productId} className="rounded-xl border border-slate-200 p-4">
                ...
                <div className="mt-2 grid gap-3 sm:grid-cols-2">
                  ...
                  <div className="text-sm text-slate-600">Allocated: <span className="font-medium tabular text-slate-900">{formatQty(draft.availableQty)} {formatUnit(draft.unit)}</span></div>
                  {split ? (
                    <>
                ...
                <div className="mt-3">
                  <Button
                    size="sm"
                    variant="secondary"
                    onClick={() => setClosingInputs((current) => ({ ...current, [draft.productId]: { fullUnits: '', looseQty: '' } }))}
                  >
                    Recount
                  </Button>
                </div>
